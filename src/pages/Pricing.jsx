      ...
      <section className="relative overflow-hidden border-b border-zinc-900/80 bg-[radial-gradient(circle_at_top,_rgba(244,63,94,0.12),_transparent_40%),linear-gradient(180deg,#09090b_0%,#121215_100%)]">
        <div className="mx-auto max-w-7xl px-6 py-20 lg:px-8">
          <div className="max-w-3xl">
            <p className="text-xs font-semibold tracking-[0.22em] text-rose-450 uppercase">Provider pricing</p>
      ...
      <section className="mx-auto max-w-7xl px-6 py-16 lg:px-8">
          ...
          {adPackages.map((pkg) => (
            ...
            >
              <div className="flex items-center gap-3">
                {pkg.premium ? <Crown className="h-5 w-5 text-amber-450" /> : <div className="h-2.5 w-2.5 rounded-full bg-zinc-650" />}
                <h2 className="text-xl font-semibold tracking-tight">{pkg.label}</h2>
              </div>
              
              <div className="mt-5 flex items-baseline">
                <span className="text-3xl font-serif font-bold tracking-tight text-zinc-105">
                  {formatPackagePrice(pkg, "weekly").split(" ")[0]}
                </span>
                <span className="ml-1 text-xs text-zinc-450">/ weekly</span>
              </div>
              
              <ul className="mt-6 flex-1 space-y-3.5 text-sm leading-6 text-zinc-350">
        ...
        <div className="mt-12 rounded-[32px] border border-zinc-900 bg-zinc-900/10 p-8 text-sm leading-7 text-zinc-400 shadow-xl font-light">
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            <Link to={createPageUrl("ProviderSignup")}>
              <Button className="rounded-full bg-gradient-to-r from-rose-500 to-amber-500 text-white font-semibold hover:opacity-95 px-8 h-12 shadow-lg border-0 glow-rose">
                Start provider signup
              </Button>
            </Link>
            <Link to={createPageUrl("Trust")}>
              <Button variant="outline" className="rounded-full border-zinc-800 bg-zinc-950 px-8 h-12 text-zinc-300 hover:bg-zinc-900 hover:text-white">
