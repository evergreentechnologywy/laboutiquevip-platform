...
export default function ProviderSignup() {
  ...
  const handleFileUpload = async (e, field = 'verification_documents') => {
    ...
    if (field === "photos" && files.length === 0) return;
    setUploading(true);
    try {
      if (field === "photos") {
        const currentCount = formData.photos.length;
        const remainingSlots = MAX_PROVIDER_PHOTOS - currentCount;
        if (remainingSlots <= 0) {
          setSubmitError(`You can upload up to ${MAX_PROVIDER_PHOTOS} photos.`);
          return;
        }
      ...
      setFormData(prev => ({
        ...prev,
        [field]: field === "photos"
          ? [...new Set([...prev[field], ...uploadedUrls])].slice(0, MAX_PROVIDER_PHOTOS)
          : [...prev[field], ...uploadedUrls]
      }));
      if (field === "photos" && files.length < Array.from(e.target.files || []).length) {
        setSubmitError(`Only the first ${MAX_PROVIDER_PHOTOS} photos were kept.`);
      }
    } catch (error) {
          ...
          {step === 3 && (
            <Card className="rounded-[28px] border-stone-200 shadow-sm overflow-hidden">
              <CardHeader className="bg-stone-50/50 border-b border-stone-100">
                ...
                <CardDescription>Upload up to 8 high-quality photos</CardDescription>
              </CardHeader>
              <CardContent className="p-8 space-y-6">
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                  ...
                  <label className="aspect-[3/4] rounded-2xl border-2 border-dashed border-stone-200 flex flex-col items-center justify-center cursor-pointer hover:bg-stone-50 transition-colors">
                    <input type="file" multiple accept="image/*" className="hidden" onChange={e => handleFileUpload(e, 'photos')} disabled={uploading || formData.photos.length >= MAX_PROVIDER_PHOTOS} />
                    {uploading ? <Loader2 className="w-6 h-6 animate-spin text-stone-400" /> : <Upload className="w-6 h-6 text-stone-300" />}
                    <span className="mt-2 text-[10px] font-semibold text-stone-400 uppercase tracking-wider">{formData.photos.length >= MAX_PROVIDER_PHOTOS ? "Max Reached" : "Add Photo"}</span>
                  </label>
                </div>
                <p className="text-xs text-stone-500">
                  {formData.photos.length}/{MAX_PROVIDER_PHOTOS} photos selected.
                </p>
                <div className="flex justify-between pt-4">
                  <Button variant="ghost" onClick={prevStep} className="text-stone-500">
                    <ArrowLeft className="mr-2 h-4 w-4" /> Back
